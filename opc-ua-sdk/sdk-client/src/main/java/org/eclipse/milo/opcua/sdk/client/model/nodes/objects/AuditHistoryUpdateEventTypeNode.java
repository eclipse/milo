...
public class AuditHistoryUpdateEventTypeNode extends AuditUpdateEventTypeNode implements AuditHistoryUpdateEventType {
    ...
    @Override
    public NodeId getParameterDataTypeId() throws UaException {
        PropertyTypeNode node = getParameterDataTypeIdNode();
        return (NodeId) node.getValue().getValue().getValue();
    }

    @Override
    public void setParameterDataTypeId(NodeId parameterDataTypeId) throws UaException {
        PropertyTypeNode node = getParameterDataTypeIdNode();
        node.setValue(new Variant(parameterDataTypeId));
    }
    ...
    @Override
    public void writeParameterDataTypeId(NodeId parameterDataTypeId) throws UaException {
        try {
            writeParameterDataTypeIdAsync(parameterDataTypeId).get();
        } catch (ExecutionException | InterruptedException e) {
            throw UaException.extract(e).orElse(new UaException(StatusCodes.Bad_UnexpectedError, e));
        }
    }
    ...
    @Override
    public CompletableFuture<? extends NodeId> readParameterDataTypeIdAsync() {
        return getParameterDataTypeIdNodeAsync().thenCompose(node -> node.readAttributeAsync(AttributeId.Value)).thenApply(v -> (NodeId) v.getValue().getValue());
    }
