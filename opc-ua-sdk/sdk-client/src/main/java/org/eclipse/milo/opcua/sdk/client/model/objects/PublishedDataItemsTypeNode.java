...
public class PublishedDataItemsTypeNode extends PublishedDataSetTypeNode implements PublishedDataItemsType {
    public PublishedDataItemsTypeNode(OpcUaClient client, NodeId nodeId, NodeClass nodeClass,
    ...
    @Override
    public PublishedVariableDataType[] getPublishedData() throws UaException {
        PropertyTypeNode node = getPublishedDataNode();
        return cast(node.getValue().getValue().getValue(), PublishedVariableDataType[].class);
    }
    ...
    @Override
    public CompletableFuture<? extends PublishedVariableDataType[]> readPublishedDataAsync() {
        return getPublishedDataNodeAsync()
            .thenCompose(node -> node.readAttributeAsync(AttributeId.Value))
            .thenApply(v -> cast(v.getValue().getValue(), PublishedVariableDataType[].class));
    }
