...
public class AlarmRateVariableTypeNode extends BaseDataVariableTypeNode implements AlarmRateVariableType {
    public AlarmRateVariableTypeNode(OpcUaClient client, NodeId nodeId, NodeClass nodeClass,
    ...
    @Override
    public UShort getRate() throws UaException {
        PropertyTypeNode node = getRateNode();
        return (UShort) node.getValue().getValue().getValue();
    }

    @Override
    public void setRate(UShort value) throws UaException {
        PropertyTypeNode node = getRateNode();
    ...
    @Override
    public CompletableFuture<? extends UShort> readRateAsync() {
        return getRateNodeAsync()
            .thenCompose(node -> node.readAttributeAsync(AttributeId.Value))
            .thenApply(v -> (UShort) v.getValue().getValue());
    }

    @Override
    public CompletableFuture<StatusCode> writeRateAsync(UShort rate) {
        DataValue value = DataValue.valueOnly(new Variant(rate));
        return getRateNodeAsync()
