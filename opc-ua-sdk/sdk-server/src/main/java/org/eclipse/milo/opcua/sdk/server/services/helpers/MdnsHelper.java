...
public class MdnsHelper	implements Runnable {
	...
	private final String MDNS_LISTENING = "_opcua-tcp._tcp.local.";
	...
	public MdnsHelper(String listeningHost, int serverPort) {
		try {
			...
			logger.error("Could not initialize address for instantiation of JmDNS for host: " + listeningHost + ". " + e.getMessage());
			e.printStackTrace();
		}
		ownPort = serverPort;
		try {
	...
	public boolean addRecord(String mdnsName, int port, String endpointName, String[] capabilities) {
		...
		ServiceInfo serviceInfo = ServiceInfo.create(MDNS_LISTENING, mdnsName, port, txt);
		try {
			jmdns.registerService(serviceInfo);
		} catch (IOException e) {
			logger.error("Could not add mDNS record for: " + mdnsName + " on port " + port + " with endpoint path: " + endpointName + ". Error: " + e.getMessage());
			return false;
		}
		return true;
	}
	...
	public boolean removeRecord(String mdnsName, int port, String endpointName) {
		...
		ServiceInfo serviceInfo = ServiceInfo.create(MDNS_LISTENING, mdnsName, port, "path=/" + endpointName);
	...
	private void processRecord(ServiceInfo serviceInfo) {
		...
		if (!serverOnNetworkMap.containsKey(serverName)) {
			...
			for (InetAddress mdnsIa : addressList) {
				...
				discoveryUrl = "opc.tcp://" + mdnsIa.getHostAddress() + ":" + serviceInfo.getPort() + serviceInfo.getPropertyString("path");
			...
			entry.serverOnNetwork = new ServerOnNetwork(UInteger.valueOf(++lastServerOnNetworkId),serverName, discoveryUrl, capsString.split(","));
